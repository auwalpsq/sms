...
@Route(value = "payment-types", layout = MainLayout::class)
class PaymentsTypeView(private val paymentTypeService: PaymentTypeService) : VerticalLayout() {
    ...
    init {
        ...
        configureGrid()
        val addButton =
                Button("Add Payment Type", Span().apply { addClassNames("ph", "ph-plus") }).apply {
    ...
    private fun configureGrid() {
        ...
        grid.addColumn { FormatUtil.formatCurrency(it.amount) }.setHeader("Amount").isAutoWidth =
                true

        grid.addColumn(PaymentType::description).setHeader("Description").isAutoWidth = true

        grid.asSingleSelect().addValueChangeListener { it.value?.let { dialog.open(it) } }
