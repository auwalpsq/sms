...
@Route("admin/applicant", layout = MainLayout::class)
...
class ApplicantReviewView(
    ...
    private fun renderApplicantPage(applicant: Applicant) {
        ...
        val backButton =
                Button(Span().apply { addClassNames("ph", "ph-arrow-left") }).apply {
                    addThemeVariants(ButtonVariant.LUMO_TERTIARY_INLINE)
                    addClickListener { UI.getCurrent().navigate("admin/manage-applications") }
                }

        val header =
                HorizontalLayout(backButton, H2("Review Admission Application")).apply {
        ...
        val applicantForm =
                FormLayout().apply {
                    ...
                    addFormItem(
                            Span(applicant.relationshipToGuardian.name),
        ...
        // Action buttons (status-driven). Do NOT add Assign here — assignment logic handled
        // separately below.
        val actions =
                    ...
                    if (applicant.applicationStatus == Applicant.ApplicationStatus.PENDING) {
                        ...
                        when {
                            ...
                            !applicant.isComplete() -> {
                                ...
                                // assignment controls will be added below when we inspect student +
                                // assignment state
                            }
                        }
                    } else /* REJECTED */ {
        ...
        launchUiCoroutine {
            ...
            if (currentSession == null) {
                ...
                // Only consider assignment UI when applicant is approved + paid + (optionally)
                // complete
                ...
                if (applicant.applicationStatus == Applicant.ApplicationStatus.APPROVED &&
                    ...
                    val assignment =
                            if (student != null) {
                                studentClassAssignmentService.findAssignment(
                                        student.id!!,
                                        currentSession.id
                                )
                            } else {
                    ...
                    ui?.withUi {
                        if (assignment != null) {
                            ...
                            actions.add(
                                    Button("Drop Admission").apply {
                                        ...
                                        addClickListener {
                                            launchUiCoroutine {
                                                try {
                                                    studentClassAssignmentService.deleteAssignment(
                                                            assignment.id!!
                                                    )
                                                    ui?.get()?.withUi {
                                                        showSuccess(
                                                                "Admission dropped successfully"
                                                        )
                            ...
                            if (applicant.isComplete()) {
                                ...
                                applicantForm.addFormItem(
                                        Span(
                                                "Application is paid but missing required details. Complete the profile before assignment."
                                        ),
                                        "Admission Status"
                                )
                            }
                        ...
                        onAssigned = { selectedClass ->
                            launchUiCoroutine {
                                try {
                                    ...
                                    ui?.withUi {
                                        showSuccess(
                                                "Assigned ${applicant.getFullName()} to ${selectedClass.name}"
                                        )
